...
import { LIST_PRODUCTS } from '../redux2/actions/productActions';
...
const Navbar = ({openSidebar}) => {
  ...
  useEffect(()=>{
    ...
    useEffect(()=>{
        dispatch(LIST_PRODUCTS(searchTerm))
    ...
    return (
        <NavWrapper fixed={isScrolled}>
            <Nav>
                ...
                <Center>
                    <Input onChange={(e)=>setSearchTerm(e.target.value)}placeholder="Search for any product by name,company,category.." autoFocus/>
                    <SearchContainer><FaSearch/></SearchContainer>
                </Center>
                <Links>{userInfo?
